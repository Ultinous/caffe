...
#include "caffe/ultinous/head_pose_data.h"
...
HeadPoseDataLayer<Dtype>::~HeadPoseDataLayer<Dtype>() {
  this->StopInternalThread();
}

template <typename Dtype>
void HeadPoseDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void HeadPoseDataLayer<Dtype>::ShuffleImages() {
...
template <typename Dtype>
void HeadPoseDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    // go to the next iter
    //lines_id_++;
    lines_id_ = select_line_id_for_head_pose_direction();
...
template <typename Dtype>
cv::Point3f HeadPoseDataLayer<Dtype>::get_random_head_pose_direction()
{
...
template <typename Dtype>
int HeadPoseDataLayer<Dtype>::select_line_id_for_head_pose_direction()
{

    unsigned seed = std::chrono::system_clock::now().time_since_epoch().count();
    std::mt19937 generator (seed);
    std::uniform_real_distribution<float> uniform01(0.0, 1.0);
    cv::Point3f p_rand = get_random_head_pose_direction();
    ...
    for(size_t i = 0; i < lines_.size(); i++)
    {
        ...
        head_direction.x = lines_[i].second[2];
        head_direction.y = lines_[i].second[5];
...
INSTANTIATE_CLASS(HeadPoseDataLayer);
REGISTER_LAYER_CLASS(HeadPoseData);
