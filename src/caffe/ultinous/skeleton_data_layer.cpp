...
struct DatumToSkeletonDecoder {
  ...
  SkeletonMetaDataType datumToMetaData(const Datum &datum) const {
    ...
    for (int i = 0; i < numberOfJointsInDatum; i++) {
      ...
      meta.targetSkeletons[0].isVisible[i] = visibilityConvert(isVisible, meta.targetSkeletons[0].joints[i], width,
                                                               height);
    ...
    for (int p = 0; p < numOtherPeople; p++) {
      ...
      for (int i = 0; i < numberOfJointsInDatum; i++) {
        meta.targetSkeletons[p + 1].joints[i].x = decode<float>(data, metaOffset +
                                                                      (9 + numOtherPeople + 3 * p + 0) * rowOffset +
                                                                      4 * i);
        meta.targetSkeletons[p + 1].joints[i].y = decode<float>(data, metaOffset +
                                                                      (9 + numOtherPeople + 3 * p + 1) * rowOffset +
                                                                      4 * i);
        ...
        meta.targetSkeletons[p + 1].isVisible[i] = visibilityConvert(isVisible, meta.targetSkeletons[p + 1].joints[i],
                                                                     width, height);
...
void SkeletonProtoDataSource<Image, MetaData, Mask>::current(Image &img, MetaData &meta, Mask &mask) {
  ...
  auto const filename = boost::filesystem::path(filenames_[current_index_]).stem().string();
  auto const maskName = (boost::format("%s/%s.%s") % mask_root_ % filename % param_.mask_extension()).str();
  mask = cv::imread(maskName, cv::IMREAD_GRAYSCALE);
/*
  if (mask.size().empty())
    LOG(INFO) << maskName << " not found.";
  else
    LOG(INFO) << maskName << " found.";
*/
...
template<
  ...
  typename Mask
>
typename MetaData::Point SkeletonProtoDataSource<Image, MetaData, Mask>::getTargetPosition(
  const typename MetaData::SkeletonType &skeleton) const {
  const auto x = std::accumulate(skeleton.joints.begin(), skeleton.joints.end(), 0.0,
                                 [](const float a, const typename MetaData::Point b) { return a + b.x; }) /
                 skeleton.joints.size();
  const auto y = std::accumulate(skeleton.joints.begin(), skeleton.joints.end(), 0.0,
                                 [](const float a, const typename MetaData::Point b) { return a + b.y; }) /
                 skeleton.joints.size();
...
template<
  ...
  typename MaskType
>
struct HeatmapCreator : public ILabelGenerator<Dtype, CoordinateType> {
  ...
  explicit HeatmapCreator(const caffe::SkeletonHeatmapGenerator &in_params,
                          const std::vector<SkeletonPointType> &pointList,
                          const int max_stride)
    : params(in_params)
    , points(pointList)
    , max_stride(max_stride)
  {
    hasBackgroundChannel = params.add_background_channel() || (params.background_channel() != caffe::SkeletonHeatmapGenerator::NONE);
    if (hasBackgroundChannel && params.background_channel() == caffe::SkeletonHeatmapGenerator::NONE)
    {
      params.set_background_channel(caffe::SkeletonHeatmapGenerator::ONE_MINUS_MAX);
    }
  }

  std::vector<int> getLabelShape(const std::vector<int>& img_shape) const override
  {
    return {
      img_shape[0],
      int(points.size()) + int(hasBackgroundChannel),
      img_shape[2] / max_stride,
      img_shape[3] / max_stride,
    };
  }

  std::size_t
  generate(Dtype *output, const Skeletons &skeletons, const int width, const int height) override
  {
    const auto gridX = width / max_stride;
    const auto gridY = height / max_stride;
    ...
    for (std::size_t i = 0; i < points.size(); ++i) {
      for (const auto &skel : skeletons) {
        ...
        if (target != skel.end()) {
          if (params.kernel_type() == caffe::SkeletonHeatmapGenerator::GAUSSIAN)
            putGaussian(output + i * channelSize, target->second, max_stride, width / max_stride, height / max_stride,
                        params.sigma());
          else if (params.kernel_type() == caffe::SkeletonHeatmapGenerator::DISK)
            putDisk(output + i * channelSize, target->second, max_stride, width / max_stride, height / max_stride, params.radius());
        }
      }
    }
    if (hasBackgroundChannel) {
      auto *data = output + points.size() * channelSize;
      if (params.background_channel() == caffe::SkeletonHeatmapGenerator::ONE_MINUS_MAX) {
      ...
      } else if (params.background_channel() == caffe::SkeletonHeatmapGenerator::MASK) {
        for (int g_y = 0; g_y < gridY; g_y++) {
          for (int g_x = 0; g_x < gridX; g_x++) {
            const auto ind = g_y * gridX + g_x;
            data[ind] = Dtype(1);
            for (std::size_t i = 0; i < points.size(); ++i) {
              if (output[i * channelSize + ind] > 0) {
                data[ind] = Dtype(0);
                break;
              }
            }
    ...
    return points.size() + static_cast<std::size_t>(hasBackgroundChannel);
  }

private:
  void putGaussian(Dtype *output, const CoordinateType &center, const int stride, const int gridX, const int gridY,
                   const Dtype sigma) const {
  ...
  void putDisk(Dtype *output, const CoordinateType &center, const int stride, const int gridX, const int gridY,
               const Dtype radius) const {
    const auto grid_center = stride / 2.0 - 0.5;
    for (int g_y = 0; g_y < gridY; g_y++) {
      for (int g_x = 0; g_x < gridX; g_x++) {
        const auto x = grid_center + g_x * stride;
        ...
        if (d2 <= radius * radius) {
          const auto index = g_y * gridX + g_x;
          output[index] = aggregator(output[index], Dtype(1));
          if (output[index] > Dtype(1))
            output[index] = Dtype(1);
        }
      }
    }
  }

  caffe::SkeletonHeatmapGenerator params;
  const std::vector<SkeletonPointType> &points;
  const std::function<Dtype(Dtype, Dtype)> aggregator = [](Dtype a, Dtype b) { return (a + b); };

  bool hasBackgroundChannel;
  int max_stride;
};
...
{
  size_t generate(Dtype *output, const MaskType &mask, const int number)
  {
    const auto channelSize = mask.rows * mask.cols;
    std::fill(output, output + number * channelSize, Dtype(0));
    for (std::size_t i = 0; i < number; ++i) {
      auto *data = output + i * channelSize;
...
template<
  ...
  typename CoordinateType
>
struct PAFCreator : public ILabelGenerator<Dtype, CoordinateType> {
  using typename ILabelGenerator<Dtype, CoordinateType>::SkeletonPointType;
  using typename ILabelGenerator<Dtype, CoordinateType>::Skeletons;
#ifdef USE_OPENCV
  ...
  explicit PAFCreator(
    ...
    , const std::vector<std::pair<SkeletonPointType, SkeletonPointType>> &edgeList
    , int max_stride
  )
    : params(params)
    , edges(edgeList)
      ...
      for (const auto &skel : skeletons) {
        ...
        if (uPoint != skel.end() && vPoint != skel.end()) {
          putVecMaps(output + i * 2 * channelSize, output + (i * 2 + 1) * channelSize, count,
                     uPoint->second, vPoint->second, max_stride, gridX, gridY, params.vec_distance_threshold());
        }
      }
    }
    return edges.size() * 2;
...
private:
  void putVecMaps(Dtype *entryX, Dtype *entryY, CounterType &count, const CoordinateType centerA,
                  const CoordinateType centerB, const int stride, const int gridX, const int gridY,
                  const int distThresh) const {
    ...
    const auto min_x = std::max(
      boost::numeric_cast<int>(std::round(std::min(stridedCenterA.x, stridedCenterB.x) - distThresh)), 0);
    const auto max_x = std::min(
      boost::numeric_cast<int>(std::round(std::max(stridedCenterA.x, stridedCenterB.x) + distThresh)), gridX);

    const auto min_y = std::max(
      boost::numeric_cast<int>(std::round(std::min(stridedCenterA.y, stridedCenterB.y) - distThresh)), 0);
    const auto max_y = std::min(
      boost::numeric_cast<int>(std::round(std::max(stridedCenterA.y, stridedCenterB.y) + distThresh)), gridY);
  ...
  : BasePrefetchingDataLayer<Dtype>(param)
{
  ...
  if (skeleton_data_param.has_db_data_param())
    ...
    this->data_source_.reset(
      new SkeletonProtoDataSource<ImageType, MetaData>(skeleton_data_param.skeleton_proto_data_param()));
  else
    LOG(FATAL) << "No supported data source given!";

  if (skeleton_data_param.has_output_model()) {
    std::transform(skeleton_data_param.output_model().points().begin(),
                   skeleton_data_param.output_model().points().end(), std::back_inserter(output_points),
                   [](const int p) {
                     return PointType(p);
                   }
    );
    std::transform(skeleton_data_param.output_model().edges().begin(), skeleton_data_param.output_model().edges().end(),
    ...
    std::copy_if(output_points.begin(), output_points.end(), std::back_inserter(generated_output_points),
                 [](const proto::skeleton::SkeletonPointType p) {
                   return p > proto::skeleton::SkeletonPointType::GEN_FIRST;
                 });
  ...
  data_transformer_.reset(
    new SkeletonDataTransformer<Dtype>(skeleton_data_param.skeleton_transform_param(), param.phase()));

  if (skeleton_data_param.has_skeleton_heatmap_generator()) {
    label_generators_.emplace_back(
      new HeatmapCreator<Dtype, typename MetaData::Point, cv::Mat_<uint8_t>>(
        skeleton_data_param.skeleton_heatmap_generator(),
        output_points,
        skeleton_data_param.skeleton_transform_param().stride()
      )
    );
  }
  if (skeleton_data_param.has_skeleton_paf_generator()) {
    label_generators_.emplace_back(
      new PAFCreator<Dtype, typename MetaData::Point>(
        skeleton_data_param.skeleton_paf_generator(),
        output_edges,
        skeleton_data_param.skeleton_transform_param().stride()
      )
    );
...
{
  const bool add_mask = true;
  if (label_generators_.empty())
  {
    return {};
  }
  else
  {
    auto result = label_generators_.front()->getLabelShape(dataSize);
    result[0] = 1;
    result[1] = 0;
    for (const auto &gen : label_generators_)
    {
      const auto current = gen->getLabelShape(dataSize);
      CHECK_EQ(current[2], result[2]);
      CHECK_EQ(current[3], result[3]);
      result[1] += current[1];
      if (add_mask)
        result[1] += current[1];
    }
...
void SkeletonDataLayer<Dtype>::load_batch(Batch<Dtype> *batch) {
  ...
  for (int item_id = 0; item_id < batchSize; ++item_id) {
    ...
    if (top_label) {
      ...
      for (auto &generator : label_generators_)
      {
        this->transformed_label_.set_cpu_data(top_label + offset_label + channelIndex * channelSize);
        const auto added = generator->generate(this->transformed_label_.mutable_cpu_data(), target,
                                               bufferRGB_.cols, bufferRGB_.rows);

        MaskGenerator<Dtype, cv::Mat_<uint8_t>> maskGen;
        maskGen.generate(this->transformed_label_.mutable_cpu_data() + added * channelSize, bufferMask_, added);
        channelIndex += added * 2;
      }
    }
