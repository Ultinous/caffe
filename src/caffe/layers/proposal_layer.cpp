                                ...
                                const vector< Blob< Dtype >* > &top )
{
  ...
  for(std::size_t i=0; i < base_anchors.size(); ++i)
    for(std::size_t j=0; j< base_anchors[i].size(); ++j)
      m_base_anchors.mutable_cpu_data()[m_base_anchors.offset(i,j)] = base_anchors[i][j];
...
{ 
  ...
  for(std::size_t ind=0; ind<height*width*m_num_anchors; ++ind)
  {
    int j=ind%m_num_anchors;
    int i=ind/m_num_anchors;
    
    m_anchors.mutable_cpu_data()[ind*4] = m_base_anchors.mutable_cpu_data()[j*4+0]+(i%width)*m_feat_stride;
    m_anchors.mutable_cpu_data()[ind*4+1] = m_base_anchors.mutable_cpu_data()[j*4+1]+(i/width)*m_feat_stride;
    m_anchors.mutable_cpu_data()[ind*4+2] = m_base_anchors.mutable_cpu_data()[j*4+2]+(i%width)*m_feat_stride;
    m_anchors.mutable_cpu_data()[ind*4+3] = m_base_anchors.mutable_cpu_data()[j*4+3]+(i/width)*m_feat_stride;
  }

  for(int i=0; m_indexes.shape(1);++i)
